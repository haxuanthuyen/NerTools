...
import edu.stanford.nlp.ie.crf.NER;
 ...
 * where the flags are set for what options to use for what flags. <p> To add a
 * new feature extractor, you should do the following: <ol> <li>Add a variable
 * (boolean, int, String, etc. as appropriate) to SeqClassifierFlags to mark if
 * the new extractor is turned on or its value, etc. Add it at the <i>bottom</i>
 * of the list of variables currently in the class (this avoids problems with
 * older serialized files breaking). Make the default value of the variable
 * false/null/0 (this is again for backwards compatibility).</li> <li>Add a
 * clause to the big if/then/else of setProperties(Properties) in
 * SeqClassifierFlags. Unless it is a macro option, make the option name the
 * same as the variable name used in step 1.</li> <li>Add code to
 * NERFeatureFactory for this feature. First decide which classes (hidden
 * states) are involved in the feature. If only the current class, you add the
 * feature extractor to the
 * <code>featuresC</code> code, if both the current and previous class, then
 * <code>featuresCpC</code>, etc.</li> </ol> <p> Parameters can be defined using
 * a Properties file (specified on the command-line with
 * <code>-prop</code> <i>propFile</i>), or directly on the command line. The
 * following properties are recognized: </p> <table border="1">
 * <tr><td><b>Property Name</b></td><td><b>Type</b></td><td><b>Default
 * Value</b></td><td><b>Description</b></td></tr> <tr><td> loadClassifier
 * </td><td>String</td><td>n/a</td><td>Path to serialized classifier to
 * load</td></tr> <tr><td> loadAuxClassifier
 * </td><td>String</td><td>n/a</td><td>Path to auxiliary classifier to
 * load.</td></tr> <tr><td> serializeTo</td><td>String</td><td>n/a</td><td>Path
 * to serialize classifier to</td></tr> <tr><td>
 * trainFile</td><td>String</td><td>n/a</td><td>Path of file to use as training
 * data</td></tr> <tr><td> testFile</td><td>String</td><td>n/a</td><td>Path of
 * file to use as training data</td></tr> <tr><td>
 * map</td><td>String</td><td>see below</td><td>This applies at training time or
 * if testing on tab-separated column data. It says what is in each column. It
 * doesn't apply when running on plain text data. The simplest scenario for
 * training is having words and classes in two column. word=0,answer=1 is the
 * default if conllNoTags is specified; otherwise word=0,tag=1,answer=2 is the
 * default. But you can add other columns, such as for a part-of-speech tag,
 * presences in a lexicon, etc. That would only be useful at runtime if you have
 * part-of-speech information or whatever available and are passing it in with
 * the tokens (that is, you can pass to classify CoreLabel tokens with
 * additional fields stored in them).</td></tr> <tr><td>
 * useWord</td><td>boolean</td><td>true</td><td>Gives you feature for
 * w</td></tr> <tr><td> useBinnedLength</td><td>String</td><td>null</td><td>If
 * non-null, treat as a sequence of comma separated integer bounds, where items
 * above the previous bound up to the next bound are binned
 * Len-<i>range</i></td></tr> <tr><td>
 * useNGrams</td><td>boolean</td><td>false</td><td>Make features from letter
 * n-grams, i.e., substrings of the word</td></tr> <tr><td>
 * lowercaseNGrams</td><td>boolean</td><td>false</td><td>Make features from
 * letter n-grams only lowercase</td></tr> <tr><td>
 * dehyphenateNGrams</td><td>boolean</td><td>false</td><td>Remove hyphens before
 * making features from letter n-grams</td></tr> <tr><td>
 * conjoinShapeNGrams</td><td>boolean</td><td>false</td><td>Conjoin word shape
 * and n-gram features</td></tr> <tr><td>
 * useNeighborNGrams</td><td>boolean</td><td>false</td><td>Use letter n-grams
 * for the previous and current words in the CpC clique. This feature helps
 * languages such as Chinese, but not so much for English</td></tr> <tr><td>
 * usePrev</td><td>boolean</td><td>false</td><td>Gives you feature for (pw,c),
 * and together with other options enables other previous features, such as
 * (pt,c) [with useTags)</td></tr> <tr><td>
 * useNext</td><td>boolean</td><td>false</td><td>Gives you feature for (nw,c),
 * and together with other options enables other next features, such as (nt,c)
 * [with useTags)</td></tr> <tr><td>
 * useTags</td><td>boolean</td><td>false</td><td>Gives you features for (t,c),
 * (pt,c) [if usePrev], (nt,c) [if useNext]</td></tr> <tr><td>
 * useWordPairs</td><td>boolean</td><td>false</td><td>Gives you features for
 * (pw, w, c) and (w, nw, c)</td></tr> <p> <tr><td>
 * useGazettes</td><td>boolean</td><td>false</td><td>If true, use gazette
 * features (defined by other flags)</td></tr> <tr><td>
 * gazette</td><td>String</td><td>null</td><td>The value can be one or more
 * filenames (names separated by a comma, semicolon or space). If provided
 * gazettes are loaded from these files. Each line should be an entity class
 * name, followed by whitespace followed by an entity (which might be a phrase
 * of several tokens with a single space between words). Giving this property
 * turns on useGazettes, so you normally don't need to specify it (but can use
 * it to turn off gazettes specified in a properties file).</td></tr> <tr><td>
 * sloppyGazette</td><td>boolean</td><td>false</td><td>If true, a gazette
 * feature fires when any token of a gazette entry matches</td></tr> <tr><td>
 * cleanGazette</td><td>boolean</td><td>false</td><td></td>If true, a gazette
 * feature fires when all tokens of a gazette entry match</tr> <p> <tr><td>
 * wordShape</td><td>String</td><td>none</td><td>Either "none" for no wordShape
 * use, or the name of a word shape function recognized by {@link WordShapeClassifier#lookupShaper(String)}</td></tr>
 ...
 * <tr><td> useLongSequences</td><td>boolean</td><td>false</td><td>Use plain
 * higher-order state sequences out to minimum of length or maxLeft</td></tr>
 * <tr><td> useBoundarySequences</td><td>boolean</td><td>false</td><td>Use extra
 * second order class sequence features when previous is CoNLL boundary, so
 * entity knows it can span boundary.</td></tr> <tr><td>
 * useTaggySequences</td><td>boolean</td><td>false</td><td>Use first, second,
 * and third order class and tag sequence interaction features</td></tr>
 * <tr><td> useExtraTaggySequences</td><td>boolean</td><td>false</td><td>Add in
 * sequences of tags with just current class features</td></tr> <tr><td>
 * useTaggySequencesShapeInteraction</td><td>boolean</td><td>false</td><td>Add
 * in terms that join sequences of 2 or 3 tags with the current shape</td></tr>
 * <tr><td> strictlyFirstOrder</td><td>boolean</td><td>false</td><td>As an
 * override to whatever other options are in effect, deletes all features other
 * than C and CpC clique features when building the classifier</td></tr>
 * <tr><td> entitySubclassification</td><td>String</td><td>"IO"</td><td>If set,
 * convert the labeling of classes (but not the background) into one of several
 * alternate encodings (IO, IOB1, IOB2, IOE1, IOE2, SBIEO, with a S(ingle),
 * B(eginning), E(nding), I(nside) 4-way classification for each class. By
 * default, we either do no re-encoding, or the CoNLLDocumentIteratorFactory
 * does a lossy encoding as IO. Note that this is all CoNLL-specific, and
 * depends on their way of prefix encoding classes, and is only implemented by
 * the CoNLLDocumentIteratorFactory. </td></tr>
 * <p/>
 * <tr><td> useSum</td><td>boolean</td><td>false</td><td></td></tr> <tr><td>
 * tolerance</td><td>double</td><td>1e-4</td><td>Convergence tolerance in
 * optimization</td></tr> <tr><td>
 * printFeatures</td><td>String</td><td>null</td><td>print out the features of
 * the classifier to a file based on this name (starting with feat-, suffixed
 * "-1" and "-2")</td></tr> <tr><td>
 * printFeaturesUpto</td><td>int</td><td>-1</td><td>Print out features for only
 * the first this many datums, if the value is positive. </td></tr>
 * <p/>
 * <tr><td> useSymTags</td><td>boolean</td><td>false</td><td>Gives you features
 * (pt, t, nt, c), (t, nt, c), (pt, t, c)</td></tr> <tr><td>
 * useSymWordPairs</td><td>boolean</td><td>false</td><td>Gives you features (pw,
 * nw, c)</td></tr>
 * <p/>
 * <tr><td> printClassifier</td><td>String</td><td>null</td><td>Style in which
 * to print the classifier. One of: HighWeight, HighMagnitude, Collection,
 * AllWeights, WeightHistogram</td></tr> <tr><td>
 * printClassifierParam</td><td>int</td><td>100</td><td>A parameter to the
 * printing style, which may give, for example the number of parameters to
 * print</td></tr> <tr><td> intern</td><td>boolean</td><td>false</td><td>If
 * true, (String) intern read in data and classes and feature (pre-)names such
 * as substring features</td></tr> <tr><td>
 * intern2</td><td>boolean</td><td>false</td><td>If true, intern all (final)
 * feature names (if only current word and ngram features are used, these will
 * already have been interned by intern, and this is an unnecessary
 * no-op)</td></tr> <tr><td>
 * cacheNGrams</td><td>boolean</td><td>false</td><td>If true, record the NGram
 * features that correspond to a String (under the current option settings) and
 * reuse rather than recalculating if the String is seen again.</td></tr>
 * <tr><td> selfTest</td><td>boolean</td><td>false</td><td></td></tr>
 * <p/>
 * <tr><td> noMidNGrams</td><td>boolean</td><td>false</td><td>Do not include
 * character n-gram features for n-grams that contain neither the beginning or
 * end of the word</td></tr> <tr><td>
 * maxNGramLeng</td><td>int</td><td>-1</td><td>If this number is positive,
 * n-grams above this size will not be used in the model</td></tr> <tr><td>
 * useReverse</td><td>boolean</td><td>false</td><td></td></tr> <tr><td>
 * retainEntitySubclassification</td><td>boolean</td><td>false</td><td>If true,
 * rather than undoing a recoding of entity tag subtypes (such as BIO variants),
 * just leave them in the output.</td></tr> <tr><td>
 * useLemmas</td><td>boolean</td><td>false</td><td>Include the lemma of a word
 * as a feature.</td></tr> <tr><td>
 * usePrevNextLemmas</td><td>boolean</td><td>false</td><td>Include the
 * previous/next lemma of a word as a feature.</td></tr> <tr><td>
 * useLemmaAsWord</td><td>boolean</td><td>false</td><td>Include the lemma of a
 * word as a feature.</td></tr> <tr><td>
 * normalizeTerms</td><td>boolean</td><td>false</td><td>If this is true, some
 * words are normalized: day and month names are lowercased (as for
 * normalizeTimex) and some British spellings are mapped to American English
 * spellings (e.g., -our/-or, etc.).</td></tr> <tr><td>
 * normalizeTimex</td><td>boolean</td><td>false</td><td>If this is true,
 * capitalization of day and month names is normalized to lowercase</td></tr>
 * <tr><td> useNB</td><td>boolean</td><td>false</td><td></td></tr> <tr><td>
 * useTypeSeqs</td><td>boolean</td><td>false</td><td>Use basic zeroeth order
 * word shape features.</td></tr> <tr><td>
 * useTypeSeqs2</td><td>boolean</td><td>false</td><td>Add additional first and
 * second order word shape features</td></tr> <tr><td>
 * useTypeSeqs3</td><td>boolean</td><td>false</td><td>Adds one more first order
 * shape sequence</td></tr> <tr><td>
 * useDisjunctive</td><td>boolean</td><td>false</td><td>Include in features
 * giving disjunctions of words anywhere in the left or right disjunctionWidth
 * words (preserving direction but not position)</td></tr> <tr><td>
 * disjunctionWidth</td><td>int</td><td>4</td><td>The number of words on each
 * side of the current word that are included in the disjunction
 * features</td></tr> <tr><td>
 * useDisjunctiveShapeInteraction</td><td>boolean</td><td>false</td><td>Include
 * in features giving disjunctions of words anywhere in the left or right
 * disjunctionWidth words (preserving direction but not position) interacting
 * with the word shape of the current word</td></tr> <tr><td>
 * useWideDisjunctive</td><td>boolean</td><td>false</td><td>Include in features
 * giving disjunctions of words anywhere in the left or right
 * wideDisjunctionWidth words (preserving direction but not position)</td></tr>
 * <tr><td> wideDisjunctionWidth</td><td>int</td><td>4</td><td>The number of
 * words on each side of the current word that are included in the disjunction
 * features</td></tr> <tr><td>
 * usePosition</td><td>boolean</td><td>false</td><td>Use combination of position
 * in sentence and class as a feature</td></tr> <tr><td>
 * useBeginSent</td><td>boolean</td><td>false</td><td>Use combination of initial
 * position in sentence and class (and word shape) as a feature. (Doesn't seem
 * to help.)</td></tr> <tr><td>
 * useDisjShape</td><td>boolean</td><td>false</td><td>Include features giving
 * disjunctions of word shapes anywhere in the left or right disjunctionWidth
 * words (preserving direction but not position)</td></tr> <tr><td>
 * useClassFeature</td><td>boolean</td><td>false</td><td>Include a feature for
 * the class (as a class marginal). Puts a prior on the classes which is
 * equivalent to how often the feature appeared in the training data.</td></tr>
 * <tr><td> useShapeConjunctions</td><td>boolean</td><td>false</td><td>Conjoin
 * shape with tag or position</td></tr> <tr><td>
 * useWordTag</td><td>boolean</td><td>false</td><td>Include word and tag pair
 * features</td></tr> <tr><td>
 * useLastRealWord</td><td>boolean</td><td>false</td><td>Iff the prev word is of
 * length 3 or less, add an extra feature that combines the word two back and
 * the current word's shape. <i>Weird!</i></td></tr> <tr><td>
 * useNextRealWord</td><td>boolean</td><td>false</td><td>Iff the next word is of
 * length 3 or less, add an extra feature that combines the word after next and
 * the current word's shape. <i>Weird!</i></td></tr> <tr><td>
 * useTitle</td><td>boolean</td><td>false</td><td>Match a word against a list of
 * name titles (Mr, Mrs, etc.)</td></tr> <tr><td>
 * useOccurrencePatterns</td><td>boolean</td><td>false</td><td>This is a very
 * engineered feature designed to capture multiple references to names. If the
 * current word isn't capitalized, followed by a non-capitalized word, and
 * preceded by a word with alphabetic characters, it returns
 * NO-OCCURRENCE-PATTERN. Otherwise, if the previous word is a capitalized NNP,
 * then if in the next 150 words you find this PW-W sequence, you get
 * XY-NEXT-OCCURRENCE-XY, else if you find W you get XY-NEXT-OCCURRENCE-Y.
 * Similarly for backwards and XY-PREV-OCCURRENCE-XY and XY-PREV-OCCURRENCE-Y.
 * Else (if the previous word isn't a capitalized NNP), under analogous rules
 * you get one or more of X-NEXT-OCCURRENCE-YX, X-NEXT-OCCURRENCE-XY,
 * X-NEXT-OCCURRENCE-X, X-PREV-OCCURRENCE-YX, X-PREV-OCCURRENCE-XY,
 * X-PREV-OCCURRENCE-X.</td></tr> <tr><td>
 * useTypeySequences</td><td>boolean</td><td>false</td><td>Some first order word
 * shape patterns.</td></tr> <tr><td>
 * useGenericFeatures</td><td>boolean</td><td>false</td><td>If true, any
 * features you include in the map will be incorporated into the model with
 * values equal to those given in the file; values are treated as strings unless
 * you use the "realValued" option (described below)</td></tr> <tr><td>
 * justify</td><td>boolean</td><td>false</td><td>Print out all feature/class
 * pairs and their weight, and then for each input data point, print
 * justification (weights) for active features</td></tr> <tr><td>
 * normalize</td><td>boolean</td><td>false</td><td>For the CMMClassifier (only)
 * if this is true then the Scorer normalizes scores as probabilities.</td></tr>
 * <tr><td> useHuber</td><td>boolean</td><td>false</td><td>Use a Huber loss
 * prior rather than the default quadratic loss.</td></tr> <tr><td>
 * useQuartic</td><td>boolean</td><td>false</td><td>Use a Quartic prior rather
 * than the default quadratic loss.</td></tr> <tr><td>
 * sigma</td><td>double</td><td>1.0</td><td></td></tr> <tr><td>
 * epsilon</td><td>double</td><td>0.01</td><td>Used only as a parameter in the
 * Huber loss: this is the distance from 0 at which the loss changes from
 * quadratic to linear</td></tr> <tr><td>
 * beamSize</td><td>int</td><td>30</td><td></td></tr> <tr><td>
 * maxLeft</td><td>int</td><td>2</td><td>The number of things to the left that
 * have to be cached to run the Viterbi algorithm: the maximum context of class
 * features used.</td></tr> <tr><td> maxRight</td><td>int</td><td>2</td><td>The
 * number of things to the right that have to be cached to run the Viterbi
 * algorithm: the maximum context of class features used. The maximum possible
 * clique size to use is (maxLeft + maxRight + 1)</td></tr> <tr><td>
 * dontExtendTaggy</td><td>boolean</td><td>false</td><td>Don't extend the range
 * of useTaggySequences when maxLeft is increased.</td></tr> <tr><td> numFolds
 * </td><td>int</td><td>1</td><td>The number of folds to use for
 * cross-validation.</td></tr> <tr><td> startFold
 * </td><td>int</td><td>1</td><td>The starting fold to run.</td></tr> <tr><td>
 * numFoldsToRun </td><td>int</td><td>1</td><td>The number of folds to
 * run.</td></tr> <tr><td> mergeTags
 * </td><td>boolean</td><td>false</td><td>Whether to merge B- and I-
 * tags.</td></tr> <tr><td>
 * splitDocuments</td><td>boolean</td><td>true</td><td>Whether or not to split
 * the data into separate documents for training/testing</td></tr> <tr><td>
 * maxDocSize</td><td>int</td><td>10000</td><td>If this number is greater than
 * 0, attempt to split documents bigger than this value into multiple documents
 * at sentence boundaries during testing; otherwise do nothing.</td></tr>
 * </table>
 * <p/>
 ...
 * <p/>
 * <
 * pre> DOCUMENTATION ON FEATURE TEMPLATES
 * <p/>
 * w = word t = tag p = position (word index in sentence) c = class p = paren g
 * = gazette a = abbrev s = shape r = regent (dependency governor) h = head word
 * of phrase n(w) = ngrams from w g(w) = gazette entries containing w l(w) =
 * length of w o(...) = occurrence patterns of words
 * <p/>
 * useReverse reverses meaning of prev, next everywhere below (on in macro)
 * <p/>
 * "Prolog" booleans: , = AND and ; = OR
 * <p/>
 * Mac: Y = turned on in -macro, + = additional positive things relative to
 * -macro for CoNLL NERFeatureFactory (perhaps none...) - = Known negative for
 * CoNLL NERFeatureFactory relative to -macro
 * <p/>
 * p
 * Bio: + = additional things that are positive for BioCreative - = things
 * negative relative to -macro
 * <p/>
 ...
 * --------------------------------------------------------------------- w,c Y
 * useWord 0 (useWord is almost useless with unlimited ngram features, but helps
 * a fraction in goodCoNLL, if only because of prior fiddling p,c usePosition ?
 * p=0,c useBeginSent ? p=0,s,c useBeginSent ? t,c Y useTags ++ pw,c Y usePrev +
 * pt,c Y usePrev,useTags 0 nw,c Y useNext ++ nt,c Y useNext,useTags 0 pw,w,c Y
 * useWordPairs + w,nw,c Y useWordPairs + pt,t,nt,c useSymTags ? t,nt,c
 * useSymTags ? pt,t,c useSymTags ? pw,nw,c useSymWordPairs ?
 * <p/>
 * pc,c Y usePrev,useSequences,usePrevSequences +++ pc,w,c Y
 * usePrev,useSequences,usePrevSequences 0 nc,c
 * useNext,useSequences,useNextSequences ? w,nc,c
 * useNext,useSequences,useNextSequences ? pc,nc,c
 * useNext,usePrev,useSequences,usePrevSequences,useNextSequences ? w,pc,nc,c
 * useNext,usePrev,useSequences,usePrevSequences,useNextSequences ?
 * <p/>
 * (pw;p2w;p3w;p4w),c + useDisjunctive (out to disjunctionWidth now) +++
 * (nw;n2w;n3w;n4w),c + useDisjunctive (out to disjunctionWidth now) ++++
 * (pw;p2w;p3w;p4w),s,c + useDisjunctiveShapeInteraction ? (nw;n2w;n3w;n4w),s,c
 * + useDisjunctiveShapeInteraction ? (pw;p2w;p3w;p4w),c + useWideDisjunctive
 * (to wideDisjunctionWidth) ? (nw;n2w;n3w;n4w),c + useWideDisjunctive (to
 * wideDisjunctionWidth) ? (ps;p2s;p3s;p4s),c useDisjShape (out to
 * disjunctionWidth now) ? (ns;n2s;n3s;n4s),c useDisjShape (out to
 * disjunctionWidth now) ?
 * <p/>
 * pt,pc,t,c Y useTaggySequences + p2t,p2c,pt,pc,t,c Y
 * useTaggySequences,maxLeft&gt;=2 + p3t,p3c,p2t,p2c,pt,pc,t,c Y
 * useTaggySequences,maxLeft&gt;=3,!dontExtendTaggy ? p2c,pc,c Y
 * useLongSequences ++ p3c,p2c,pc,c Y useLongSequences,maxLeft&gt;=3 ?
 * p4c,p3c,p2c,pc,c Y useLongSequences,maxLeft&gt;=4 ? p2c,pc,c,pw=BOUNDARY
 * useBoundarySequences 0 (OK, but!)
 * <p/>
 ...
 * <p/>
 * s,pc,c Y useTypeySequences ++ ns,pc,c Y useTypeySequences // error for ps?
 * not? 0 ps,pc,s,c Y useTypeySequences 0 // p2s,p2c,ps,pc,s,c Y
 * useTypeySequences,maxLeft&gt;=2 // duplicated a useTypeSeqs2 feature
 * <p/>
 * n(w),c Y useNGrams (noMidNGrams, MaxNGramLeng, lowercaseNGrams,
 * dehyphenateNGrams) +++ n(w),s,c useNGrams,conjoinShapeNGrams ?
 * <p/>
 * g,c + useGazFeatures // test refining this? ? pg,pc,c + useGazFeatures ? ng,c
 * + useGazFeatures ? // pg,g,c useGazFeatures ? // pg,g,ng,c useGazFeatures ?
 * // p2g,p2c,pg,pc,g,c useGazFeatures ? g,w,c useMoreGazFeatures ? pg,pc,g,c
 * useMoreGazFeatures ? g,ng,c useMoreGazFeatures ?
 * <p/>
 * g(w),c useGazette,sloppyGazette (contains same word) ? g(w),[pw,nw,...],c
 * useGazette,cleanGazette (entire entry matches) ?
 * <p/>
 * s,c Y wordShape &gt;= 0 +++ ps,c Y wordShape &gt;= 0,useTypeSeqs + ns,c Y
 * wordShape &gt;= 0,useTypeSeqs + pw,s,c Y wordShape &gt;= 0,useTypeSeqs +
 * s,nw,c Y wordShape &gt;= 0,useTypeSeqs + ps,s,c Y wordShape &gt;=
 * 0,useTypeSeqs 0 s,ns,c Y wordShape &gt;= 0,useTypeSeqs ++ ps,s,ns,c Y
 * wordShape &gt;= 0,useTypeSeqs ++ pc,ps,s,c Y wordShape &gt;=
 * 0,useTypeSeqs,useTypeSeqs2 0 p2c,p2s,pc,ps,s,c Y wordShape &gt;=
 * 0,useTypeSeqs,useTypeSeqs2,maxLeft&gt;=2 +++ pc,ps,s,ns,c wordShape &gt;=
 * 0,useTypeSeqs,useTypeSeqs3 ?
 * <p/>
 * p2w,s,c if l(pw) &lt;= 3 Y useLastRealWord // weird features, but work 0
 * n2w,s,c if l(nw) &lt;= 3 Y useNextRealWord ++ o(pw,w,nw),c Y
 * useOccurrencePatterns // don't fully grok but has to do with capitalized name
 * patterns ++
 * <p/>
 * a,c useAbbr;useMinimalAbbr pa,a,c useAbbr a,na,c useAbbr pa,a,na,c useAbbr
 * pa,pc,a,c useAbbr;useMinimalAbbr p2a,p2c,pa,pc,a useAbbr w,a,c useMinimalAbbr
 ...
 * <p/>
 * r,c useNPGovernor (only for baseNP words) r,t,c useNPGovernor (only for
 * baseNP words) h,c useNPHead (only for baseNP words) h,t,c useNPHead (only for
 * baseNP words)
 * <p/>
 * </pre>
 *
...
public class NERFeatureFactory<IN extends CoreLabel> extends FeatureFactory<IN> {
    ...
    public static HashSet<String> locDicHNHCM=readict("HanoiHCM.txt");
    public static HashSet<String> keyWord=readict("KeyLocWord.txt");
...
//    System.err.println(doFE+"\t"+domain);
    ...
    private void distSimAnnotate(PaddedList<IN> info) {
        for (CoreLabel fl : info) {
            if (fl.has(DistSimAnnotation.class)) {
                return;
            }
    ...
    public static HashSet<String> readict(String fileName) {
        ...
        try {
            BufferedReader br = new BufferedReader(new FileReader(new File(path_Dict + fileName)));
            String line = "";
    ...
    protected Collection<String> featuresC(PaddedList<IN> cInfo, int loc) {
        ...
        if(flags.isNumber)
        {
            char[] t=cWord.trim().toCharArray();
            boolean allNum=true;
            for(int k=0;k<t.length;k++)
            {
                if(!(Character.isDigit(t[k])|| t[k]=='/')) allNum=false;
                break;
            }
            if(allNum==true) featuresC.add("NumberAddress");
             t=pWord.trim().toCharArray();
             allNum=true;
            for(int k=0;k<t.length;k++)
            {
                if(!(Character.isDigit(t[k])|| t[k]=='/')) allNum=false;
            ...
            if(allNum==true) featuresC.add("NumberAddressP");
        }
        if(flags.useNearKeyWord)
        {
            boolean t=false;
            for(int i=1;i<=2;i++)
            {
                if(keyWord.contains(getWord(cInfo.get(loc-i)).toLowerCase().trim())) t=true;
                break;
                
            }
            if(t) featuresC.add("NearKeyWord");
        }
        if (flags.useDistSim) {
            distSimAnnotate(cInfo);
        }
        if (flags.currentWord) {
            featuresC.add(cWord + "-CWORD");
        }
        if (flags.prevWord) {
            featuresC.add(pWord + "-PWORD");
        }
        if (flags.prev2Word) {
            featuresC.add(p2Word + "-P2WORD");
        }
        if (flags.next2Word) {
            featuresC.add(n2Word + "-N2WORD");
        }
        if (flags.nextWord) {
            featuresC.add(nWord + "-NWORD");
        }
        if (flags.initUpcaseCurrentWord) {
            featuresC.add(Character.isUpperCase(cWord.charAt(0)) + "INITUPCASECWORD");
        }
        if (flags.initUpcasePrevWord) {
            featuresC.add(Character.isUpperCase(pWord.charAt(0)) + "INITUPCASEPWORD");
        }
        if (flags.initUpcasePrev2Word) {
            featuresC.add(Character.isUpperCase(p2Word.charAt(0)) + "INITUPCASEP2WORD");
        }
        if (flags.initUpcaseNextWord) {
            featuresC.add(Character.isUpperCase(nWord.charAt(0)) + "INITUPCASENWORD");
        }
        if (flags.initUpcaseNext2Word) {
            featuresC.add(Character.isUpperCase(n2Word.charAt(0)) + "INITUPCASEN2WORD");
        }
        if (flags.lowercaseCurrentWord) {
            featuresC.add(cWord + "LOWERCASECWORD");
        }
        if (flags.lowercasePrevWord) {
            featuresC.add(pWord + "LOWERCASEPWORD");
        }
        if (flags.lowercaseNextWord) {
            featuresC.add(nWord + "LOWERCASENWORD");
        }
        if (flags.wordPairPC) {
            featuresC.add(pWord + "-" + cWord + "-WordPairPC");
        }
        if (flags.wordPairCN) {
            featuresC.add(cWord + "-" + nWord + "-WordPairCN");
        }
        if (flags.wordPairPN) {
            featuresC.add(pWord + "-" + nWord + "-WordPairPN");
        }
        if (flags.allCapCurrentWord) {
            boolean all = true;
            ...
            if (all == true) {
                featuresC.add("-AllCapCurrentWord");
            }
        }
        if (flags.allCapPrevWord) {
            boolean all = true;
            ...
            if (all == true) {
                featuresC.add("-AllCapPrevWord");
            }
        }
        if (flags.allCapPrev2Word) {
            boolean all = true;
            ...
            if (all == true) {
                featuresC.add("-AllCapPrev2Word");
            }
        }
        if (flags.allCapNextWord) {
            boolean all = true;
            ...
            if (all == true) {
                featuresC.add("-AllCapNextWord");
            }
        }
        if (flags.allCapNext2Word) {
            boolean all = true;
            ...
            if (all == true) {
                featuresC.add("-AllCapNext2Word");
            }
        }
        if (flags.letterAndDigitCWord) {
            boolean hasLetter = false;
            boolean hasDigit = false;
            for (int t = 0; t < cWord.length(); t++) {
                if (Character.isAlphabetic(cWord.charAt(t))) {
                    hasLetter = true;
                }
                if (Character.isDigit(cWord.charAt(t))) {
                    hasDigit = true;
                }
            }
            if (hasLetter && hasDigit) {
                featuresC.add("-LetterAndDigit");
            }

        }
        if (flags.letterAndDigitPWord) {
            boolean hasLetter = false;
            boolean hasDigit = false;
            for (int t = 0; t < pWord.length(); t++) {
                if (Character.isAlphabetic(pWord.charAt(t))) {
                    hasLetter = true;
                }
                if (Character.isDigit(pWord.charAt(t))) {
                    hasDigit = true;
                }
            }
            if (hasLetter && hasDigit) {
                featuresC.add("-LetterAndDigitPWord");
            }

        }
        if (flags.letterAndDigitP2Word) {
            boolean hasLetter = false;
            boolean hasDigit = false;
            for (int t = 0; t < pWord.length(); t++) {
                if (Character.isAlphabetic(p2Word.charAt(t))) {
                    hasLetter = true;
                }
                if (Character.isDigit(p2Word.charAt(t))) {
                    hasDigit = true;
                }
            }
            if (hasLetter && hasDigit) {
                featuresC.add("-LetterAndDigitP2Word");
            }

        }
        if (flags.letterAndDigitNWord) {
            boolean hasLetter = false;
            boolean hasDigit = false;
            for (int t = 0; t < pWord.length(); t++) {
                if (Character.isAlphabetic(nWord.charAt(t))) {
                    hasLetter = true;
                }
                if (Character.isDigit(nWord.charAt(t))) {
                    hasDigit = true;
                }
            }
            if (hasLetter && hasDigit) {
                featuresC.add("-LetterAndDigitNWord");
            }

        }
        if (flags.letterAndDigitN2Word) {
            boolean hasLetter = false;
            boolean hasDigit = false;
            for (int t = 0; t < pWord.length(); t++) {
                if (Character.isAlphabetic(n2Word.charAt(t))) {
                    hasLetter = true;
                }
                if (Character.isDigit(n2Word.charAt(t))) {
                    hasDigit = true;
                }
            }
            if (hasLetter && hasDigit) {
                featuresC.add("-LetterAndDigitN2Word");
            }

        }

        if (flags.prefixCWord) {
            featuresC.add(cWord.substring(0, 3) + "-prefixCword");

        }
        if (flags.prefixPWord) {
            featuresC.add(pWord.substring(0, 3) + "-prefixPword");

        }
        if (flags.prefixP2Word) {
            featuresC.add(p2Word.substring(0, 3) + "-prefixP2word");

        }
        if (flags.isPunctuationPWord) {

            if (pWord.equals(".") || pWord.equals("!") || pWord.equals("?")) {
                featuresC.add("-isPunctuationPWord");
            }

        }

        if (flags.isPunctuationP2Word) {

            if (p2Word.equals(".") || p2Word.equals("!") || p2Word.equals("?")) {
                featuresC.add("-isPunctuationP2Word");
            }

        }
        if (flags.isPunctuationNWord) {

            if (nWord.equals(".") || nWord.equals("!") || nWord.equals("?")) {
                featuresC.add("-isPunctuationNWord");
            }

        }
        if (flags.isPunctuationN2Word) {

            if (n2Word.equals(".") || n2Word.equals("!") || n2Word.equals("?")) {
                featuresC.add("-isPunctuationN2Word");
            }
        ...
        if (flags.isBracket) {
            ...
            if (cWord.equals("(") || cWord.equals(")")) {
                featuresC.add("-isBracket");
            }

        }
        if (flags.isBracketPWord) {

            if (pWord.equals("(") || pWord.equals(")")) {
                featuresC.add("-isBracketPWord");
            }

        }
        if (flags.isBracketP2Word) {

            if (p2Word.equals("(") || p2Word.equals(")")) {
                featuresC.add("-isBracketP2Word");
            }

        }
        if (flags.isBracketN2Word) {

            if (n2Word.equals("(") || n2Word.equals(")")) {
                featuresC.add("-isBracketN2Word");
            }

        }

        if (flags.isBracketNWord) {

            if (nWord.equals("(") || nWord.equals(")")) {
                featuresC.add("-isBracketNWord");
            }

        }
        if (flags.isFirstSentenceWord) {

            if (pWord.equals(".") || pWord.equals("?") || pWord.equals("!")||pWord.equals(";")) {
                if (Character.isUpperCase(cWord.charAt(0))) {
                    featuresC.add("-isFirstSentenceWord");
                }
            }

        }
        if (flags.isFirstSentencePWord) {

            if (p2Word.equals(".") || p2Word.equals("?") || p2Word.equals("!")|| p2Word.equals(";")) {
                if (Character.isUpperCase(pWord.charAt(0))) {
                    featuresC.add("-isFirstSentencePWord");
                }
            }

        }

        if (flags.isFirstSentenceP2Word) {

            if (p3Word.equals(".") || p3Word.equals("?") || p3Word.equals("!")|| p3Word.equals(";") || p3Word == null) {
                if (Character.isUpperCase(p2Word.charAt(0))) {
                    featuresC.add("-isFirstSentenceP2Word");
                }
            }

        }
        if (flags.isInCountryList) {

            String w = cWord.replace('_', ' ');
            if (countryList.contains(w.trim())) {
                featuresC.add("-isInCountryList");
            }

        }
        if (flags.isInCountryListPWord) {

            String w = pWord.replace('_', ' ');
            if (countryList.contains(w.trim())) {
                featuresC.add("-isInCountryListPWord");
            }

        }
        if (flags.isInCountryListP2Word) {

            String w = p2Word.replace('_', ' ');
            if (countryList.contains(w.trim())) {
                featuresC.add("-isInCountryListP2Word");
            }

        }
        if (flags.isInCountryListN2Word) {

            String w = n2Word.replace('_', ' ');
            if (countryList.contains(w.trim())) {
                featuresC.add("-isInCountryListN2Word");
            }

        }
        if (flags.isInCountryListNWord) {

            String w = nWord.replace('_', ' ');
            if (countryList.contains(w.trim())) {
                featuresC.add("-isInCountryListNWord");
            }

        }
        if (flags.isInVnFirstName) {

            String[] part = cWord.split("_");

            if (part.length > 0) {
                if (vnFirstName.contains(part[0])) {
                    featuresC.add("-isInVnFirstName");
                }
            }


        }
        if (flags.isInVnFirstNamePWord) {

            String[] part = pWord.split("_");

            if (part.length > 0) {
                if (vnFirstName.contains(part[0])) {
                    featuresC.add("-isInVnFirstNamepWord");
                }
            }
        ...
        if (flags.isInVnFirstNameP2Word) {

            String[] part = p2Word.split("_");

            if (part.length > 0) {
                if (vnFirstName.contains(part[0])) {
                    featuresC.add("-isInVnFirstNamep2Word");
                }
            }


        }
        if (flags.isInVnFirstNameN2Word) {

            String[] part = n2Word.split("_");

            if (part.length > 0) {
                if (vnFirstName.contains(part[0])) {
                    featuresC.add("-isInVnFirstNameN2Word");
                }
            }


        }
        if (flags.isInVnFirstNameNWord) {

            String[] part = nWord.split("_");

            if (part.length > 0) {
                if (vnFirstName.contains(part[0])) {
                    featuresC.add("-isInVnFirstNameNWord");
                }
            }


        }
        if (flags.isInVnLastName) {

            String[] part = cWord.split("_");

            if (part.length > 0) {
                if (vnLastName.contains(part[part.length - 1])) {
                    featuresC.add("-isInVnLastName");
                }
            }


        }
        if (flags.isInVnLastNamePWord) {

            String[] part = pWord.split("_");

            if (part.length > 0) {
                if (vnLastName.contains(part[part.length - 1])) {
                    featuresC.add("-isInVnLastNamePWord");
                }
            }


        }
        if (flags.isInVnLastNameP2Word) {

            String[] part = p2Word.split("_");

            if (part.length > 0) {
                if (vnLastName.contains(part[part.length - 1])) {
                    featuresC.add("-isInVnLastNameP2Word");
                }
            }


        }
        if (flags.isInVnLastNameN2Word) {

            String[] part = n2Word.split("_");

            if (part.length > 0) {
                if (vnLastName.contains(part[part.length - 1])) {
                    featuresC.add("-isInVnLastNameN2Word");
                }
            }


        }
        if (flags.isInVnLastNameNWord) {

            String[] part = nWord.split("_");

            if (part.length > 0) {
                if (vnLastName.contains(part[part.length - 1])) {
                    featuresC.add("-isInVnLastNameNWord");
                }
            }
        ...
        if (flags.inLocDict) {
            ...
            if (locDict.contains(cWord.replace('_', ' '))) {
                featuresC.add("-isInLocDict");
            }


        }
        if (flags.inLocDictPWord) {


            if (locDict.contains(pWord.replace('_', ' '))) {
                featuresC.add("-isInLocDictPWord");
            }


        }
        if (flags.inLocDictP2Word) {


            if (locDict.contains(p2Word.replace('_', ' '))) {
                featuresC.add("-isInLocDictP2Word");
            }


        }
        if (flags.inLocDictN2Word) {


            if (locDict.contains(n2Word.replace('_', ' '))) {
                featuresC.add("-isInLocDictN2Word");
            }


        }
        if (flags.inLocDictNWord) {


            if (locDict.contains(nWord.replace('_', ' '))) {
                featuresC.add("-isInLocDictNWord");
            }
        ...
        if (flags.vnTimeMarkers) {


            if (vnTimeMarker.contains(cWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-vnTimeMarkers");
            }


        }
        if (flags.vnTimeMarkersPWord) {


            if (vnTimeMarker.contains(pWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-vnTimeMarkersPWord");
            }


        }
        if (flags.vnTimeMarkersP2Word) {


            if (vnTimeMarker.contains(p2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-vnTimeMarkersP2Word");
            }


        }
        if (flags.vnTimeMarkersN2Word) {


            if (vnTimeMarker.contains(n2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-vnTimeMarkersN2Word");
            }


        }
        if (flags.vnTimeMarkersNWord) {


            if (vnTimeMarker.contains(nWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-vnTimeMarkersNWord");
            }


        }
        if (flags.perIndicateNoun) {


            if (perIndicateNoun.contains(cWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-perIndicateNoun");
            }


        }
        if (flags.perIndicateNounPWord) {


            if (perIndicateNoun.contains(pWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-perIndicateNounPWord");
            }


        }
        if (flags.perIndicateNounP2Word) {


            if (perIndicateNoun.contains(p2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-perIndicateNounP2Word");
            }


        }
        if (flags.perIndicateNounN2Word) {


            if (perIndicateNoun.contains(n2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-perIndicateNounN2Word");
            }


        }
        if (flags.perIndicateNounNWord) {


            if (perIndicateNoun.contains(nWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-perIndicateNounNWord");
            }


        }
        if (flags.locIndicateNounN2Word) {


            if (locIndicateNoun.contains(n2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-locIndicateNounN2Word");
            }


        }
        if (flags.locIndicateNounNWord) {


            if (locIndicateNoun.contains(nWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-locIndicateNounNWord");
            }


        }
        if (flags.locIndicateNounPWord) {


            if (locIndicateNoun.contains(pWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-locIndicateNounPWord");
            }


        }
        if (flags.locIndicateNounP2Word) {


            if (locIndicateNoun.contains(p2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-locIndicateNounP2Word");
            }


        }
        if (flags.locIndicateNoun) {


            if (locIndicateNoun.contains(cWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-locIndicateNounCWord");
            }


        }
        if (flags.orgIndicateNoun) {


            if (orgIndicateNoun.contains(cWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-orgIndicateNoun");
            }


        }
        if (flags.orgIndicateNounPWord) {


            if (orgIndicateNoun.contains(pWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-orgIndicateNounPWord");
            }


        }
        if (flags.orgIndicateNounP2Word) {


            if (orgIndicateNoun.contains(p2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-orgIndicateNounP2Word");
            }


        }
        if (flags.orgIndicateNounN2Word) {


            if (orgIndicateNoun.contains(n2Word.replace('_', ' ').toLowerCase())) {
                featuresC.add("-orgIndicateNounN2Word");
            }


        }
        if (flags.orgIndicateNounNWord) {


            if (orgIndicateNoun.contains(nWord.replace('_', ' ').toLowerCase())) {
                featuresC.add("-orgIndicateNounNWord");
            }
        ...
        if (flags.p2Tag) {
            ...
            featuresC.add(p2.getString(PartOfSpeechAnnotation.class) + "-p2TAG");
        ...
        if (flags.n2Tag) {
            ...
            featuresC.add(n2.getString(PartOfSpeechAnnotation.class) + "-n2TAG");


        }
        if (flags.postfixCWord) {


            featuresC.add(cWord.substring(cWord.length()-3)+"-postfixCWord");


        }
        if (flags.postfixPWord) {


            featuresC.add(pWord.substring(pWord.length()-3)+"-postfixPWord");


        }
        if (flags.postfixNWord) {


            featuresC.add(nWord.substring(nWord.length()-3)+"-postfixNWord");


        }
        if(flags.useLocDictHNHCM)
        {
            if(locDicHNHCM.contains(cWord.toLowerCase())) featuresC.add("InLocDicHNHCMC");
            if(locDicHNHCM.contains(pWord.toLowerCase())) featuresC.add("InLocDicHNHCMP");
            if(locDicHNHCM.contains(p2Word.toLowerCase())) featuresC.add("InLocDicHNHCMP2");
            if(locDicHNHCM.contains(nWord.toLowerCase())) featuresC.add("InLocDicHNHCMN");
            if(locDicHNHCM.contains(n2Word.toLowerCase())) featuresC.add("InLocDicHNHCMN2");
        }
        ...
        if (flags.useInternal && flags.useExternal) {
            ...
            //tieng viet
            if (flags.usePerLocAmbiguityDict) {
                if (NER.PER_LOC_AMBIGUTY_DICT.contains(pWord.toLowerCase()) || NER.PER_LOC_AMBIGUTY_DICT.contains(nWord.toLowerCase())
                        || NER.PER_LOC_AMBIGUTY_DICT.contains(n2Word.toLowerCase())) {
                    featuresC.add("PerLocAmbiguityDict");
                }



            }
            if (flags.useNumberUpcasePrev) {
                int num = 0;
                ...
                featuresC.add(num + "-NumberPrevUpcase");
            }
            if (flags.useNumberUpcaseNext) {
                int num = 0;
                for (int i = 0; i < nWord.length(); i++) {
                    if (Character.isUpperCase(nWord.charAt(i))) {
                        num++;
                    }
                }
                featuresC.add(num + "-NumberNextUpcase");
            }
            if (flags.usePrev2WordLocIndicateAdverbDict) {
                featuresC.add(NER.LOC_INDICATE_ADVERB_DICT.contains(p2Word) + "usePrev2WordLocIndicateAdverbDict");
            }
            if (flags.usePrev2WordLocIndicateVerbDict) {
                featuresC.add(NER.LOC_INDICATE_VERB_DICT.contains(p2Word) + "usePrev2WordLocIndicateVerbDict");
            }
            if (flags.useNext2WordPerIndicateNounDict) {
                featuresC.add(NER.PER_INDICATE_NOUN_DICT.contains(n2Word) + "useNext2WordPerIndicateNounDict");
            }
            if (flags.useConJunctionDict) {
                featuresC.add(NER.CONJUNCTION_DICT.contains(pWord) + "PrevWordConjunction");
                featuresC.add(NER.CONJUNCTION_DICT.contains(p2Word) + "Prev2WordConjunction");
                featuresC.add(NER.CONJUNCTION_DICT.contains(nWord) + "NextWordConjunction");
                featuresC.add(NER.CONJUNCTION_DICT.contains(n2Word) + "Next2WordConjunction");
            ...
            if (flags.useNumberUpcase) {
                int num = 0;
                for (int i = 0; i < cWord.length(); i++) {
                    if (Character.isUpperCase(cWord.charAt(i))) {
                        num++;
                    }
                }

                featuresC.add(num + "-NumberUpcaseC");
                num = 0;
                for (int i = 0; i < pWord.length(); i++) {
                    if (Character.isUpperCase(pWord.charAt(i))) {
                        num++;
                ...
                featuresC.add(num + "-NumberUpcaseP");
                num = 0;
                for (int i = 0; i < p2Word.length(); i++) {
                    if (Character.isUpperCase(p2Word.charAt(i))) {
                        num++;
                    }
                }
                featuresC.add(num + "-NumberUpcaseP2");
                num = 0;
                for (int i = 0; i < nWord.length(); i++) {
                    if (Character.isUpperCase(nWord.charAt(i))) {
                        num++;
                ...
                featuresC.add(num + "-NumberUpcaseN");
                num = 0;
                for (int i = 0; i < n2Word.length(); i++) {
                    if (Character.isUpperCase(n2Word.charAt(i))) {
                        num++;
                    }
                }
                featuresC.add(num + "-NumberUpcaseN2");

            }

            if (flags.useInitUpcasePrev) {
                if (pWord.length() > 0) {
                    featuresC.add(Character.isUpperCase(pWord.charAt(0)) + "InitUpCasePrev");
                }
            }
            if (flags.useInitUpcase) {
                if (cWord.length() > 0 && !pWord.equalsIgnoreCase(".") && Character.isUpperCase(cWord.charAt(0))) {
                    featuresC.add("InitUpCase");
                }
            ...
            if (flags.useInitUpcaseNext) {
                if (nWord.length() > 0) {
                    featuresC.add(Character.isUpperCase(nWord.charAt(0)) + "InitUpCaseNext");
                }


            }
            if (flags.usePerIndicateNounDict) {
//        
                if (NER.PER_INDICATE_NOUN_DICT.contains(pWord.toLowerCase()) || NER.PER_INDICATE_NOUN_DICT.contains(nWord.toLowerCase())
                        || NER.PER_INDICATE_NOUN_DICT.contains(n2Word.toLowerCase())) {
                    featuresC.add("PerIndicateNounDict");
                }
            }
            if (flags.useOrgIndicateNounDict) {
//         
                if (NER.ORG_INDICATE_NOUN_DICT.contains(pWord.toLowerCase())) {
                    featuresC.add("useOrgIndicate");
                }
            }
            if (flags.useLocIndicateVerbDict) {
                if (NER.LOC_INDICATE_VERB_DICT.contains(pWord.toLowerCase())) {
                    featuresC.add("-useLocindicateVerb");
                }

            }
            if (flags.useLocIndicateNounDict) {
                if (NER.LOC_INDICATE_NOUN_DICT.contains(pWord.toLowerCase())) {
                    featuresC.add("-useLocindicateNoun");
                }
            }
            if (flags.useLocIndicateAdverbDict) {
                featuresC.add(NER.LOC_INDICATE_ADVERB_DICT.contains(cWord.toLowerCase()) + "useLocIndicateAdverbDict");
                featuresC.add(NER.LOC_INDICATE_ADVERB_DICT.contains(pWord.toLowerCase()) + "useLocIndicateAdverbDictPrev");
                featuresC.add(NER.LOC_INDICATE_ADVERB_DICT.contains(p2Word.toLowerCase()) + "useLocIndicateAdverbDictPrev2");
                featuresC.add(NER.LOC_INDICATE_ADVERB_DICT.contains(nWord.toLowerCase()) + "useLocIndicateAdverbDictNext");
                featuresC.add(NER.LOC_INDICATE_ADVERB_DICT.contains(n2Word.toLowerCase()) + "useLocIndicateAdverbDictNext2");

            }
            if (flags.useDefineDict) {
                if (NER.DEFINE_DICT.contains(pWord.toLowerCase()) || NER.DEFINE_DICT.contains(nWord.toLowerCase())) {
                    featuresC.add("-useDefineDict");
                }
            }
            if (flags.useLocDict) {
                if (NER.LOC_DICT.contains(cWord.toLowerCase())) {
                    featuresC.add("-useLocDict");
                }
            }
            if (flags.usePrevWordDefineDict) {
                featuresC.add(NER.DEFINE_DICT.contains(pWord) + "usePrevWordDefineDict");
            }
            if (flags.usePrevWordInOrgIndicateNounDict) {
                featuresC.add(NER.ORG_INDICATE_NOUN_DICT.contains(pWord) + "usePrevWordInOrgIndicateNounDict");
            }
            if (flags.usePrevWordLocIndicateAdverbDict) {
                featuresC.add(NER.LOC_INDICATE_ADVERB_DICT.contains(pWord) + "usePrevWordLocIndicateAdverbDict");
            }
            if (flags.usePrevWordLocIndicateNounDict) {
                featuresC.add(NER.LOC_INDICATE_NOUN_DICT.contains(pWord) + "usePrevWordLocIndicateNounDict");
            }
            if (flags.usePrevWordLocIndicateVerbDict) {
                featuresC.add(NER.LOC_INDICATE_VERB_DICT.contains(pWord) + "usePrevWordLocIndicateVerbDict");
            }
            if (flags.usePrevWordPerIndicateNounDict) {
                featuresC.add(NER.PER_INDICATE_NOUN_DICT.contains(pWord) + "usePrevWordPerIndicateNounDict");
            }
...
//         String cShape = c.get(ShapeAnnotation.class);
    ...
    private Collection<String> occurrencePatterns(PaddedList<IN> cInfo, int loc) {
        ...
        /*
         * if (!l.isEmpty()) { System.err.println(pWord+" "+word+" "+nWord+"
         * "+l); }
         */
        return l;
    }
