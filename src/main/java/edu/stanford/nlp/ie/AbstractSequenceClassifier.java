...
//      printResults(entityTP, entityFP, entityFN);
  ...
  public static boolean countResults(List<? extends CoreMap> doc,
    ...
    for (CoreMap line : doc) {
      ...
      System.out.println("Ti le phan loai dung: "+total/doc.size());
      try {
            //test ghi ra file 
            FileWriter fw=new FileWriter("E:\\Fscore.txt");
            PrintWriter out=new PrintWriter(fw);
            out.print(total/doc.size());
            out.close();
        } catch (IOException ex) {
            Logger.getLogger(AbstractSequenceClassifier.class.getName()).log(Level.SEVERE, null, ex);
        }
