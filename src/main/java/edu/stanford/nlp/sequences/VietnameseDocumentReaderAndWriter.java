 ...
 * @author Kho
 */
public class VietnameseDocumentReaderAndWriter implements DocumentReaderAndWriter<CoreLabel> {
...
//  }
  ...
  public void init(SeqClassifierFlags flags) {
    this.map = StringUtils.mapStringToArray(flags.map);
    factory = DelimitRegExIterator.getFactory("\n(?:\\s*\n)+", new VietnameseDocumentReaderAndWriter.ColumnDocParser());
  }
  ...
  private class ColumnDocParser implements Serializable, Function<String,List<CoreLabel>> {
    ...
    public List<CoreLabel> apply(String doc) {
      ...
      for (String line : lines) {
        ...
        //sua doi cho tieng viet, xoa cac dau [].''
        for(int i=0;i<info.length;i++)
        {
            info[i]=info[i].replace("[", "").replace("]", "").trim();
        }
        
        //ket thuc sua doi cho tieng viet
